...
```{r}
dscf <- dscfTest(
  response = GameData$copies,
  factor = GameData$production
)
```
...
knitr::kable( 
  x = dscf,
  digits = 3, 
  col.names = c("Comparison", "Observed W", "Adj. p-value"),     caption = paste("Post Hoc-Dwass-Steel-Critchlow-Fligner            Tests"), 
...
kw.PostHoc( 
    response = GameData$copies, 
    treatments = GameData$production 
  ) %>% 
knitr::kable( 
  digits = 3, 
  caption = "Post Hoc Comparison Effect Sizes", 
  col.names = c("Pairwise Comparison","Hodges Lehmann Estimate", "Prob. Superiority"), 
  align = 'lcc',
  booktabs = TRUE 
) %>% 
  kableExtra::kable_styling( 
